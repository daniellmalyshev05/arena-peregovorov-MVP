...
/**
 * Аудит играбельности сценария.
 *
 * Проверяет не «работает ли код», а есть ли в кейсе чему учиться: существует ли
 * зона соглашения, наказывается ли позиционный торг, есть ли разрыв приоритетов,
 * не устраивает ли статус-кво обе стороны сразу.
 *
 * Это же ядро работает и в админке: администратор настраивает контекст и сразу
 * видит доказательство, что сценарий играбелен, — до того как посадить туда человека.
 * Считается перебором, детерминированно, без сети.
 */
