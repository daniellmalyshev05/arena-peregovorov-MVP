...
export function score(scenario: Scenario, state: NegotiationState): ScoreReport {
  ...
  // Предложить можно только условия, выведенные в разговор, поэтому выход
  // оценивается по достижимому набору, а не по полному перебору.
  ...
  const discovery = scenario.hiddenInterests.length
    ...
    : 1
  // Выход обоснован, если из открытого ничего не собиралось. Качество решения
  // показывает глубина разведки (discovery).
  ...
  // Эталон — половина совместной ценности, а не теоретический максимум:
  // выжать оппонента до точки отказа — не цель.
  ...
  if (walkedAway && noZopa) {
    ...
    // 8 — балл за выход от достижимой сделки: обоснованный выход не может стоить
    // меньше. Остальные 17 — пропорционально доле раскрытых интересов.
...
function diagnose(
  ...
  if (state.status === 'timeout') {
    ...
    return {
      // Исход один, причина разная: раунды кончились сами или игрок закончил разговор.
  ...
  if (state.status === 'walkaway') {
    ...
    if (!reachableZopa) {
      // Из открытого сделки не было — уйти было верно. Но если интересы
      // не раскрыты, выход верен лишь формально, и заголовок не хвалит.
      return {
    ...
    // Пакет собирался из открытых условий, поэтому показываем его самого,
    // а не нераскрытый интерес.
  ...
  // Ценности создано много, но игроку досталось меньше 60% справедливой доли:
  // такую сделку нельзя называть сильной.
