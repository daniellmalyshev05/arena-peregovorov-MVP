...
/**
 * Оппонент, который растёт вместе с игроком (ТЗ §2.4: рост сложности).
 *
 * Движок читает профиль и делает оппонента жёстче там, где игрок слаб:
 * уступаете без встречного условия — он заберёт и попросит ещё; не задаёте
 * вопросов — не станет помогать; выигрываете стабильно — садится за стол
 * собранным.
 */
...
/** `title` и `note` — для экрана профиля, `instruction` — для модели. */
const TARGETS: Record<AdaptationTarget, { title: string; cause: string; note: string; instruction: string }> = {
  ...
  shallow: {
    ...
    cause: 'вы редко доходили до её настоящих интересов',
 ...
 *
 * Прогноз после первой зачётной сессии; сама адаптация включается со второй.
 */
