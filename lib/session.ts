...
/**
 * Незаконченная сессия в браузере.
 *
 * ТЗ требует, чтобы демо не ломалось по ходу показа. Обновление страницы
 * посреди партии стирало разговор целиком: состояние жило только в памяти
 * вкладки. Теперь живая сессия переживает перезагрузку, а с ней — и снапшоты,
 * на которых держится возврат.
 *
 * Хранится только НЕЗАКОНЧЕННАЯ партия: разбор и развилка не восстанавливаются,
 * им нужны полный расчёт и история, и подделывать это состояние нечестно.
 */
