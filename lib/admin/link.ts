 ...
 *
 * Зачем это есть. Контур администратора хранит конфигурацию в браузере: без
 * сервера она не доезжает до участника, и на вопрос «как ваша настройка
 * попадает к обучаемому» честного ответа не было. Ссылка отвечает на него без
 * бэкенда: вся конфигурация лежит в самом адресе, участник открывает его у себя
 * и попадает ровно в ту симуляцию, которую собрал администратор.
 *
 * Кодируется не вся конфигурация, а её отличия от библиотечного кейса. Если
 * поменяли только сложность, в ссылке едут четыре байта, а не весь бриф.
 */
interface Packed {
  /** Кейс-основа. */
  b: string
