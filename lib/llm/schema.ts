...
export function parseLlmTurn(raw: string): ParseOutcome {
  ...
  // Самая частая поломка живого прогона — не «модель ответила не то»,
  // а «модель не дописала»: ответ обрывается на полуслове, закрывающей скобки
  // нет. Целый ход при этом выбрасывать нельзя — реплика в нём уже есть.
 ...
 * Всё лучше, чем заготовка запасного движка невпопад.
 */
