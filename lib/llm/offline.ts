...
export function detectActs(text: string): SpeechAct[] {
  ...
  // Матчим по корням слов, а не по целым фразам: живая речь почти никогда
  // не совпадает с заготовленным шаблоном дословно.
  if (isQuestion && has(t, 'произойдет', 'что будет', 'чем обернет', 'приведет', 'обойдет', 'последств', 'если сдвин', 'если не успе', 'риску', 'потеря', 'сорвет')) {
    acts.push('spin_implication')
  }
  if (isQuestion && has(t, 'мешает', 'сложност', 'трудност', 'проблем', 'риск', 'не устраивает', 'столкнул', 'недобрал', 'буксует', 'болев', 'узкое место')) {
    acts.push('spin_problem')
  }
  if (isQuestion && has(t, 'как сейчас', 'расскажите', 'как устроен', 'какой график', 'сколько', 'как проходит', 'какие сроки', 'как обстоят', 'что у вас с', 'опишите')) {
    acts.push('spin_situation')
  }
  if (isQuestion && has(t, 'что бы вам дало', 'насколько важно', 'что если бы', 'помогло бы', 'ценно', 'решило бы', 'закрыло бы', 'снимет ли', 'устроит ли')) {
    acts.push('spin_needpayoff')
  }
  if (has(t, 'кто принимает', 'совет директоров', 'согласован', 'полномоч', 'от кого зависит', 'кто утвержда', 'кто подписыва')) {
    acts.push('authority_check')
  }
  if (has(t, 'правильно ли я пон', 'то есть вы', 'если я вас верно', 'вы говорите, что', 'услышал', 'верно ли, что', 'иными словами')) {
    acts.push('active_listening')
  }
  if (has(t, 'если мы', 'взамен', 'при условии', 'в обмен', 'тогда вы', 'готовы, если', 'при том что', 'в ответ')) {
    acts.push('conditional_offer')
  }
  if (has(t, 'по рынку', 'в среднем', 'статистик', 'индекс', 'данные', 'практика показ', 'регламент', 'норматив', 'по отрасли')) {
