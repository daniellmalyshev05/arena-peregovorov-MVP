 ...
 *   2. страховка, если сеть или провайдер недоступны;
 *   3. фолбэк, когда ответ модели не удалось разобрать.
 *
 * Он проще живой модели, но не падает и не нарушает правила мира.
 */
 ...
 *
 * Матчинг устроен двумя приёмами:
 ...
 *      «кто у вас подписывает» это один и тот же вопрос.
 *
 * По этой разметке запасной движок выбирает реплику (см. scripts/check-acts.ts).
 */
 ...
 *
 * Порядок важен: сначала реакция на то, что игрок сказал по существу, и только
 * потом — общая реплика.
 */
