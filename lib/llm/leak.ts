 ...
 *
 * Движок не засчитывает раскрытие, если речевой акт не подошёл, но модель
 * может проговорить секрет прозой, оставив `revealedInterests` пустым.
 * После каждого хода реплика сверяется с тем, что на этот момент раскрыто
 * НЕ было; при достаточном совпадении в консоль сервера уходит предупреждение
 * с цитатой.
 *
 * Считается детерминированно, без обращений наружу.
 */
...
/**
 * Основа слова: первые пять букв. При шести короткие существительные
 * («совет») выпадали бы целиком, а секрет часто проговаривается именно ими.
 */
const STEM = 5
...
export function detectLeak(scenario: Scenario, state: NegotiationState, reply: string): Leak[] {
   ...
   * слова секрета звучат законно, утечку выдаёт порядок слов.
   */
