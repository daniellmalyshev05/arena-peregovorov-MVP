...
export function patterns(runs: RunRecord[]): Pattern[] {
  ...
  if (withUnilateral / n >= 0.5) {
    ...
    // Считается по сессиям, а не по сумме ходов: обмен без голых уступок
    // минимум в двух сессиях и в 75% из них. Предикат — тот же, что у навыка.
...
/**
 * Навыки переговорщика как допуски, а не как полоска опыта (ТЗ §2.4).
 *
 * Навык — это повторяемость: он освоен, когда подтвердился в двух зачётных
 * сессиях, и опирается на те же числа, которыми считается разбор.
 */
...
export function skills(runs: RunRecord[]): Skill[] {
  ...
  const build = (
    ...
    text: {
      untested: string
      /** Что показала единственная сессия. Порог освоения от неё не меняется. */
    ...
    // Одна сессия навык не закрывает, но её результат показываем сразу,
    // а не заглушку «не проверялся».
  ...
  return [
    ...
    build(
      ...
      (r) => r.facts >= 2,
      {
        ...
        mastered: (hits) => `Вы опираетесь на факты: подтверждено в ${sessions(hits)}.`,
      },
    ),
    build(
      'model',
