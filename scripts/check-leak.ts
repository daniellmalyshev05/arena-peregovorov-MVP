...
// 4b. Перефраз с живого прогона: модель не цитирует секрет, а пересказывает его.
//     Детектор остаётся эвристикой: он ловит близкий пересказ, а не любой
//     синоним. Дальше этого без второго вызова модели не уехать.
{
  const s = scenarios[0]
  const state = createInitialState(s)
  const paraphrases: { line: string; id: string }[] = [
    {
      // 17 и 19 сентября Виктор произносил это в ответ на пустую вежливость.
      line:
