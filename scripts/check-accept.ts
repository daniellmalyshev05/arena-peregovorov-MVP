/**
 * Согласие второй стороны всегда доходит до игрока.
 *
 * Дыра, которую закрывает эта проверка: пакет могли принять, а стол при этом
 * закрыт меньше чем на нужную долю. Движок оставлял состояние живым, кнопки
 * закрытия не появлялось, и вторая сторона вслух звала юристов на экране, где
 * ничего не происходило. Человек, играющий без подсказки рядом, читал это как
 * поломку ровно в момент своего успеха.
 *
...
// Ранняя игра: условий на столе меньше трёх, и «нужно N» в этот момент не
// существует — порог бесконечен. Строка, которую видит игрок, обязана быть для
// этого случая своей. На первом же прогоне эта ветка выдала «сдвинуто 1 условий
// из Infinity»: числа в интерфейсе нельзя брать из движка не глядя.
