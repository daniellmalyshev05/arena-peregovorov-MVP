...
/**
 * Шаги разбора. Четвёртый называется по тому, что на нём показано: ошибка
 * или, если её нет, поворотный ход. «Кто это посчитал» стоит сразу за баллом.
 */
...
/** Анимированный счётчик итогового балла. */
function useCountUp(value: number, duration = 900) {
  ...
  useEffect(() => {
    ...
    // В фоновой вкладке кадры не приходят, а таймер срабатывает: итог
    // выставится, даже если анимация не отыграла.
...
/**
 * Разбор партии: шесть шагов, на каждом — один вопрос.
 * Сначала причина, метрики — вторым слоем.
 */
export function Debrief({
  ...
  // Без соглашения совместной ценности нет: вся возможная осталась на столе.
  ...
  /**
   * Кто что решил в этой партии: счёт на числах сыгранной партии.
   * Вердикт и баллы модель не трогает; разметку актов и настроение — в границах,
   * которые проверяет движок.
   */
  ...
  /**
   * Ход годится в «где вы ошиблись», только если таймлайн не помечает его
   * находкой. Часть кандидатов на откат выбирается формулой и может попасть
   * на ход, который как раз раскрыл интерес.
   */
  ...
  const fault = [
    score.rootCauseTurn,
    // Штраф с привязкой к ходу — тоже ошибка (например, переход на личность).
  ...
  return (
    <main className="flex h-dvh flex-col bg-paper">
        ...
        {STEPS.map((label: string, i: number) => {
          ...
          return (
            <button
              key={label}
              onClick={() => setStep(i)}
              // Номер и подпись — разные спаны, поэтому имя для скринридера задаём явно.
      ...
      <div className="h-full overflow-y-auto px-6 py-8 lg:px-14">
        ...
        {/* Шаг 3 — кто считал: экономика, вердикты и баллы — код, модель играет
            человека. Показано на числах сыгранной партии. */}
        {step === 2 && (
          <div className="rise mx-auto max-w-[1180px]">
            <div className="lbl mb-3">{STEPS[2]}</div>
            <h1 className="max-w-[820px] text-h2 font-semibold tracking-[-0.016em] text-balance">
              Ваш балл посчитан по формулам
            </h1>
            <p className="mt-3.5 max-w-[720px] text-lead text-ink2 text-pretty">
              ...
              и сказать она может только то, что уже решено. Поэтому уговорить модель на лучший
              исход не получится: результат зависит только от условий пакета.
            </p>
              ...
              <div className="rounded-lg border border-accent-line bg-accent-soft p-5 sm:p-6">
                ...
                <dl className="flex flex-col gap-3.5">
                  {[
                    ...
                    /* До 640 px пояснение уходит под число во всю ширину. */
                    <div
                      key={r.t}
        ...
        {step === 5 && (
          <div className="rise mx-auto max-w-[1180px]">
            ...
            {/* Вернуть можно любой ход, не только предложенные моменты. */}
