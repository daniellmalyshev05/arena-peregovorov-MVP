...
/**
 * Справка «Как это устроено».
 *
 * Два пробела закрываются одной страницей. Первый: методики, на которых всё
 * построено, нигде не названы — внутри движка они есть в каждой формуле, но
 * человек со стороны не может этого увидеть. Второй: показатели результата
 * появляются только в разборе, то есть когда партия уже сыграна вслепую.
 *
 * Страница, а не модальное окно: её можно открыть до игры, вернуться к ней
 * после и дать ссылку другому человеку. Модальное окно закрывают не читая.
 */
...
const METHODS = [
  {
    ...
    here: [
      ...
      'Условия обмениваются пакетом, а не по одному: на одном условии выгода не создаётся, она появляется на разнице приоритетов.',
  ...
  },
  {
    name: 'BATNA',
    summary:
      'Лучшая альтернатива переговорному соглашению. Она задаёт нижнюю границу приемлемого: сделка хуже своей альтернативы — это не сделка, а проигрыш.',
    here: [
      'Запасной вариант посчитан в баллах у обеих сторон. Он определяет, что вторая сторона примет, а что отвергнет, — и решает это код, а не модель.',
...
const SCORE = [
  { label: 'Ценность сделки относительно запасного варианта', max: 25, note: 'Нормируется не к максимуму, а к половине создаваемой ценности: выжать вторую сторону досуха — не хорошие переговоры.' },
  ...
  { label: 'Использование объективных критериев', max: 10, note: 'Факты, приложенные к месту, а не все подряд.' },
    ...
    <main className="min-h-dvh bg-paper">
      ...
      <div className="mx-auto max-w-[900px] px-6 py-10 sm:px-8 sm:py-14">
        ...
        <h1 className="mt-3.5 max-w-[620px] text-h1 font-semibold tracking-[-0.02em] text-balance">
          Переговоры здесь считаются, а не оцениваются на глаз
        </h1>
        ...
        <section className="mt-12 border-t border-line pt-9">
          ...
          <p className="mt-2.5 max-w-[640px] leading-relaxed text-ink2 text-pretty">
            Три классические рамки переговоров. Здесь они не пересказаны в виде теории — они зашиты
            в механику, и каждая отвечает за свою часть стола.
          </p>
