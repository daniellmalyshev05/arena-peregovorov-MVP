 ...
 *
 * Показывает открытые условия и число ещё не выведенных в разговор. Названия
 * неоткрытых условий скрыты: иначе панель подсказывает, о чём спрашивать.
 */
export function DealPanel({
  ...
  // Счётчик оценок досье виден всегда и подсвечен, пока не оценено ничего:
  // всплывающая подсказка покрывает не все утверждения.
  ...
  return (
    <aside className={`min-h-0 flex-col border-l border-line bg-surface ${className || 'flex'}`}>
      <div className="flex h-[46px] shrink-0 items-center gap-5 border-b border-line px-4 lg:px-[18px]">
        ...
        {/* До xl — короткая подпись: длинная не помещается в колонку на 1024 px. */}
