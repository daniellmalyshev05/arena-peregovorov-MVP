 ...
 */
export function Mark({ size = 24, animate = false }: { size?: number; animate?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" aria-hidden focusable="false">
      ...
      {/* На первом экране граница возможного прочерчивается, а точка сделки
          садится на неё следом: тот же порядок, что и на карте в разборе. */}
      <path
        d="M7.5 7C7.5 15.5 16.5 24.5 25 24.5"
        stroke="var(--color-paper)"
        strokeWidth="2.2"
        strokeLinecap="round"
        className={animate ? 'mark-draw' : undefined}
      />
      <g className={animate ? 'mark-dot' : undefined}>
